...
public class CJone extends JFrame implements ActionListener{
	...
	public CJone(String s)
	{
		...
		try {
			...
			//查询语句
			String sql="select (@i:=@i+1) 序号,num,班级,name,C语言,面向对象的程序设计,Java,总分 \r\n" + 
					"from\r\n" + 
					"(select * from 计科1701 union select * from 计科1702 order by 总分 desc) as c ,(SELECT @i:=0) AS 序号\r\n" + 
					"where 班级=\r\n" + 
					"(select 班级 from (select * from 计科1701 union select * from 计科1702) as c where num=?)";
	...
	public void actionPerformed(ActionEvent arg0)
	{
		if(arg0.getSource()==jbselect)
		{
			...
			CJselect cjs=new CJselect(str,strname);
		}
